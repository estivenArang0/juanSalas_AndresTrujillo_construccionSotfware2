...
public class UserRepositoryAdapter implements UserRepository {
    ...
    @Override public Optional<User> findByIdentificationNumber(String id) { return jpa.findByIdIdentificacion(id).map(mapper::toDomain); }
    @Override public List<User> findAll() { return jpa.findAll().stream().map(mapper::toDomain).collect(Collectors.toList()); }
    @Override public boolean existsByIdentificationNumber(String id) { return jpa.existsByIdIdentificacion(id); }
